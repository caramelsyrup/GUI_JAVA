...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		...
		btnConfirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					int pw = Integer.parseInt(tfPwd.getText());
					new LoginTest(id,pw);
				}catch(NumberFormatException e1) {
					JOptionPane.showMessageDialog(null, "Á¤º¸¸¦ ÀÔ·ÂÇÏ¼¼¿ä.");
				}
			}	
		});
		
		btnConfirm.setFont(new Font("¸¼Àº °íµñ", Font.PLAIN, 15));
		btnConfirm.setBounds(186, 376, 107, 29);
		panel1.add(btnConfirm);
		
		JLabel lblPassword = new JLabel("PASSWORD");
		lblPassword.setFont(new Font("¸¼Àº °íµñ", Font.PLAIN, 20));
		...
		panel1.add(tfPwd);
		btnCreate.setFont(new Font("¸¼Àº °íµñ", Font.PLAIN, 15));
